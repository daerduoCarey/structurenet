...
from data_partnetobb import PartNetObbDataset, Tree, collate_feats
...
# load unit cube pc
unit_cube = torch.from_numpy(load_pts('cube.pts')).to(device)

def boxLoss(box_feature, gt_box_feature):
    pred_box_pc = provider.transform_pc_batch(unit_cube, box_feature)
    pred_reweight = provider.get_surface_reweighting_batch(box_feature[:, 3:6], unit_cube.size(0))
    gt_box_pc = provider.transform_pc_batch(unit_cube, gt_box_feature)
    gt_reweight = provider.get_surface_reweighting_batch(gt_box_feature[:, 3:6], unit_cube.size(0))
        ...
        if pred_node.is_leaf:
            ...
            for sem in intersect_sem:
                ...
                if num_gt == 1 and num_pred == 1:
                    ...
                    _, cur_matched_gt_idx, cur_matched_pred_idx = linear_assignment(dmat)
            ...
            if gt_node.edges is not None:
                ...
                for edge in gt_node.edges:
                    ...
                    if gt_part_a_id in matched_gt_idx and gt_part_b_id in matched_gt_idx:
                        ...
                        mesh_v1, mesh_f1 = provider.gen_obb_mesh(obb1)
                        pc1 = provider.sample_pc(mesh_v1, mesh_f1, n_points=500)
                        ...
                        mesh_v2, mesh_f2 = provider.gen_obb_mesh(obb2)
                        pc2 = provider.sample_pc(mesh_v2, mesh_f2, n_points=500)
...
with torch.no_grad():
    for batch_ind, batch in enumerate(dataloader):
        ...
        root_code_and_kld = encoder.encode_structure(obj=obj, eval_time=True)
        ...
        orig_output_filename = os.path.join(conf.result_path, conf.name, obj_name, 'orig', 'object.json')
        recon_output_filename = os.path.join(conf.result_path, conf.name, obj_name, 'recon', 'object.json')
        ...
        PartNetObbDataset.save_object(obj=obj, fn=orig_output_filename)
        PartNetObbDataset.save_object(obj=recon_obj, fn=recon_output_filename)

        # chamfer distance
        ori_obbs_np = torch.cat([item.view(1, -1) for item in obj.boxes(leafs_only=True)], dim=0).cpu().numpy()
        ori_mesh_v, ori_mesh_f = provider.gen_obb_mesh(ori_obbs_np)
        ori_pc_sample = provider.sample_pc(ori_mesh_v, ori_mesh_f)
        recon_obbs_np = torch.cat([item.view(1, -1) for item in recon_obj.boxes(leafs_only=True)], dim=0).numpy()
        recon_mesh_v, recon_mesh_f = provider.gen_obb_mesh(recon_obbs_np)
        recon_pc_sample = provider.sample_pc(recon_mesh_v, recon_mesh_f)
        cd1, cd2 = chamfer_loss(torch.tensor(ori_pc_sample, dtype=torch.float32).view(1, -1, 3), 
                torch.tensor(recon_pc_sample, dtype=torch.float32).view(1, -1, 3))
        cd = ((cd1.sqrt().mean() + cd2.sqrt().mean()) / 2).item()
        chamfer_dists.append(cd)

        stat_filename = os.path.join(conf.result_path, conf.name, obj_name, 'stats.txt')
    ...
    dataset_stat_filename = os.path.join(conf.result_path, conf.name, 'stats.txt')
